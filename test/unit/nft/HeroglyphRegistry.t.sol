...
import {
  ...
  Ownable,
  LiteTickerFarmPool
} from "src/services/nft/HeroglyphRegistry.sol";

import { FailOnReceive } from "test/mock/contract/FailOnReceive.t.sol";

contract HeroglyphRegistryTest is BaseTest {
  ...
  HeroglyphRegistryHarness underTest;

  function setUp() external {
    _setupMockVariables();
    _setupMockCalls();

    underTest = new HeroglyphRegistryHarness(owner, treasury, hctMock, nftPassMock, dripVaultMock, dataAsserterMock);
  }
  ...
  function test_addToCollection_whenZeroValue_thenReverts() external prankAs(user) {
    vm.expectRevert(IHeroglyphRegistry.ZeroValue.selector);
  ...
  function test_addToCollection_whenOverRequiredETH_thenReverts() external prankAs(user) {
    vm.expectRevert(IHeroglyphRegistry.TooManyEth.selector);
  ...
  function test_addToCollection_whenGoalNotReached_thenAddsEthAndDepositsIntoDripVault() external prankAs(user) {
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.CollectionContributed(collectionMock, user, givingAmount);
  ...
  function test_addToCollection_whenGoalReached_thenCreatesWrappedNFT() external prankAs(user) {
    ...
    emit IHeroglyphRegistry.WrappedNFTCreated(collectionMock, address(0));
  ...
  function test_createWrappedNFT_thenVerifyWrappedNFTConfiguration() external {
    ...
    assertEq(address(wrappedNFT.heroglyphRegistry()), address(underTest));
  ...
  function test_removeFromCollection_whenAmountExceedsDeposit_thenReverts() external prankAs(user) {
    vm.expectRevert(IHeroglyphRegistry.AmountExceedsDeposit.selector);
  ...
  function test_removeFromCollection_whenGoalReached_thenReverts() external prankAs(user) {
    ...
    vm.expectRevert(IHeroglyphRegistry.GoalReached.selector);
    underTest.removeFromCollection(collectionMock, 1);
  }
  ...
  function test_removeFromCollection_whenTransferFails_thenReverts() external prankAs(user) {
    ...
    vm.expectRevert(IHeroglyphRegistry.TransferFailed.selector);
    underTest.removeFromCollection(collectionMock, 1);
  }
  ...
  function test_removeFromCollection_whenAmountIsZero_thenRemovesAll() external prankAs(user) {
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.CollectionContributionWithdrawn(collectionMock, user, givingAmount);
  ...
  function test_removeFromCollection_whenGoalNotReached_thenRemovesAmount() external prankAs(user) {
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.CollectionContributionWithdrawn(collectionMock, user, withdrawn);
  ...
  function test_supportYieldPool_thenUpdatesSupportersAndDepositInDripVault() external prankAs(user) {
    ...
    IHeroglyphRegistry.Supporter memory expectedSupporter = IHeroglyphRegistry.Supporter({
      depositor: user,
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.Supported(1, user, supportAmount);
  ...
  function test_retrieveSupportToYieldPool_whenNotSupporter_thenReverts() external prankAs(user) {
    vm.expectRevert(IHeroglyphRegistry.NotSupporterDepositor.selector);
  ...
  function test_retrieveSupportToYieldPool_whenSupportNotFinished_thenReverts() external prankAs(user) {
    ...
    vm.expectRevert(IHeroglyphRegistry.SupportNotFinished.selector);
    underTest.retrieveSupportToYieldPool(1);
  }
  ...
  function test_retrieveSupportToYieldPool_whenAlreadyRemoved_thenReverts() external prankAs(user) {
    ...
    vm.expectRevert(IHeroglyphRegistry.AlreadyRemoved.selector);
    underTest.retrieveSupportToYieldPool(1);
  }
  ...
  function test_retrieveSupportToYieldPool_thenWithdrawsFromDripVault() external prankAs(user) {
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.SupportRetrieved(1, user, supportAmount);
    underTest.retrieveSupportToYieldPool(1);
  ...
  function test_setTickerLogic_thenUpdatesTickerPool() external prankAs(owner) {
    ...
    expectExactEmit();
    emit IHeroglyphRegistry.TickerLogicSet(ticker, expectedPool);
  ...
  function test_addNewGenesisTicker_whenTickerExists_thenReverts() external prankAs(owner) {
    ...
    vm.expectRevert(IHeroglyphRegistry.TickerAlreadyExists.selector);
    underTest.addNewGenesisTicker(ticker, mockGenesisWrappedToken, mockGenesisKey);
  }
  ...
  function test_addNewGenesisTicker_thenCreatesNewTickerPool() external prankAs(owner) {
    ...
    vm.expectEmit(true, false, false, false);
    emit IHeroglyphRegistry.NewGenesisTickerCreated(ticker, address(0));
  ...
  function test_createContract_whenFailedDeployment_thenReverts() external {
    vm.expectRevert(IHeroglyphRegistry.FailedDeployment.selector);
    underTest.exposed_createContract(type(HeroglyphRegistry).creationCode);
  }

  function test_allowNFTCollection_asNonOwner_thenReverts() external prankAs(user) {
    vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, user));
    underTest.allowNFTCollection(collectionMock);
  }

  function test_allowNFTCollection_whenTooManyEth_thenReverts() external pranking {
    changePrank(user);
    underTest.addToCollection{ value: REQUIRED_ETH_TO_ENABLE_COLLECTION }(collectionMock);
  ...
  function test_allowNFTCollection_whenNoContribution_thenAddsToAllowedCollections() external prankAs(owner) {
    changePrank(owner);
    vm.expectEmit(true, false, false, false);
    emit IHeroglyphRegistry.WrappedNFTCreated(collectionMock, address(0));
    underTest.allowNFTCollection(collectionMock);
  ...
  function test_allowNFTCollection_whenSomeContribution_thenAddsToAllowedCollections() external pranking {
    changePrank(user);
...
contract HeroglyphRegistryHarness is HeroglyphRegistry {
  constructor(
    address _owner,
    address _treasury,
    address _hct,
  ...
  ) HeroglyphRegistry(_owner, _treasury, _hct, _nftPass, _dripVault, _dataAsserter) { }
