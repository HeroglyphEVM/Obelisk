 ...
 * @dev It is in it's standalone as we might later on change the name filtering logic,
 * allowing or removing unicodes
 */
contract NameFilter {
    ...
    returns (bool, uint256 index)
  {
    ...
    while (index < strBytes.length) {
      ...
      if (charByte <= 0x7F) {
        ...
        if (
          !(charByte > 0x20 && charByte <= 0x7E) || charByte == 0xA0 || charByte == 0x23
            || charByte == 0x24 || charByte == 0x3A || charByte == 0x2C || charByte == 0x40
            || charByte == 0x2D
        ) {
          return (false, index);
        }
        ...
        charValue = (uint16(uint8(strBytes[index]) & 0x1F) << 6)
          | (uint16(uint8(strBytes[index + 1])) & 0x3F);
        if (
        ...
        charValue = (uint16(uint8(strBytes[index]) & 0x0F) << 12)
          | (uint16(uint8(strBytes[index + 1]) & 0x3F) << 6)
          | (uint16(uint8(strBytes[index + 2])) & 0x3F);
        if (
